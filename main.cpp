...
std::vector<std::size_t> tag_sort(const std::vector<Type>& vec)
{
    std::vector<std::size_t> result(vec.size());
...
//        children_bots.clear();
        ...
        for (int i = 0; i < allBots.size(); i++) {
            // simulate the timeframe
            while(allBots[i].robotTime < simTimeLength) { allBots[i].simulate(); }
            // find the current value
